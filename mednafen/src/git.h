...
typedef struct
{
	// Pointer to framebuffer, size pitch * fb_height, set by the driver code.  Framebuffer written to by system emulation code.
	//uint32 *pixels;
	...
	// Will be set to TRUE if the video format has changed since the last call to Emulate(), FALSE otherwise.
...
typedef enum
{
 MODPRIO_INTERNAL_LOW = 0,
 MODPRIO_EXTERNAL_LOW = 1,
 MODPRIO_INTERNAL_HIGH = 2,
 MODPRIO_EXTERNAL_HIGH = 3
} ModPrio;
 ...
 */
 ...
 // Insert emulated disk/disc.
 int (*DiskInsert)(int oride);

 // Eject current inserted emulated disk/disc.
 int (*DiskEject)(void);

 // Select current emulated disk/disc for insertion.
 int (*DiskSelect)(void);

 bool (*SetSoundRate)(uint32 Rate);
 ...
 int pitch;		// Pitch/Width of the framebuffer(not necessarily width of the image)
