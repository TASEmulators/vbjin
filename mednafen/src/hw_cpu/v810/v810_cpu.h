...
class V810
{
 ...
 // Pass TRUE for vb_mode if we're emulation a VB-specific enhanced V810 CPU core
 ...
 #ifdef WANT_DEBUGGER
 INLINE void SetMemPeekHandlers(uint8 (*peek8)(const v810_timestamp_t, uint32), uint16 (*peek16)(const v810_timestamp_t, uint32), uint32 (*peek32)(const v810_timestamp_t, uint32))
 {
  MemPeek8 = peek8;
  MemPeek16 = peek16;
  MemPeek32 = peek32;
 ...
 #ifdef WANT_DEBUGGER
 void CheckBreakpoints(void (*callback)(int type, uint32 address, unsigned int len));
 ...
 void Run_Accurate_Debug(int32 MDFN_FASTCALL (*event_handler)(const v810_timestamp_t timestamp)) __attribute__((noinline));
 #endif

 v810_timestamp_t next_event_ts;

 uint8 *v810_fast_map[(1ULL << 32) / V810_FAST_MAP_PSIZE];
 void *v810_fast_map_nf[(1ULL << 32) / V810_FAST_MAP_PSIZE];
 ...
 #ifdef WANT_DEBUGGER
 uint8 (*MemPeek8)(v810_timestamp_t timestamp, uint32 A);
 uint16 (*MemPeek16)(v810_timestamp_t timestamp, uint32 A);
 uint32 (*MemPeek32)(v810_timestamp_t timestamp, uint32 A);
 #endif
 ...
 void CacheOpMemStore(uint32 A, uint32 V);
 uint32 CacheOpMemLoad(uint32 A);

 void CacheClear(uint32 start, uint32 count);
 void CacheDump(const uint32 SA);
 void CacheRestore(const uint32 SA);

 uint32 RDCACHE(uint32 addr);
 //
 // End caching related
 //

 uint16 RDOP(uint32 addr, uint32 meow = 2);
 void SetFlag(uint32 n, bool condition);
 void SetSZ(uint32 value);

 void SetSREG(unsigned int which, uint32 value);
